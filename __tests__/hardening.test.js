const { DetectionEngine, Normalizer, expressMiddleware } = require('../src/index');
...
describe('Security hardening regressions', () => {
  ...
  it('detects SQL block comments used as whitespace between keywords', () => {
    ...
    const decoded = Normalizer.decodeDeeply(payload);
  ...
  it('keeps standalone metadata catalog mentions below the default block threshold', () => {
    const result = engine.detect('The all_tables view is documented in Oracle.');
  ...
  it('detects payloads through deeper nested URL encoding', () => {
    ...
    const result = engine.detect(encoded);
  ...
  it('keeps long pathological-looking inputs within a broad timing budget', () => {
    ...
    for (const sample of samples) {
      expect(() => engine.detect(sample)).not.toThrow();
    }
  ...
  it('detects bare destructive DDL without requiring a semicolon', () => {
    const result = engine.detect('DROP TABLE users');
  ...
  it('does not treat ordinary prose containing "drop table" as DDL', () => {
    expect(engine.detect('Drop table near the window.').label).toBe('benign');
  });
  ...
  it('does not block a JSON-ish quoted equals value as SQLi', async () => {
    ...
    expect(engine.detect(req.body.payload).label).toBe('benign');
  ...
  it('keeps weak single-signal findings below the default block threshold', () => {
    const result = engine.detect('javascript:');
  ...
  it('blocks javascript URLs in URL-bearing HTML attributes', () => {
    const result = engine.detect('<a href="javascript:globalThis[\'al\'+\'ert\'](1)">x</a>');
  ...
  it('blocks javascript URLs that route through constructor chains before the sink', () => {
    const result = engine.detect('javascript:[]["constructor"]["constructor"]("alert(1)")()');
...
describe('Express middleware hardening options', () => {
  it('records detections without blocking when dryRun is enabled', async () => {
    ...
    expect(res.status).not.toHaveBeenCalled();
    expect(next).toHaveBeenCalledTimes(1);
    expect(req.fortifyjs).toEqual(expect.objectContaining({
      detected: true,
      dryRun: true,
      label: 'sqli'
    }));
    expect(onThreat).toHaveBeenCalledWith(req.fortifyjs, req);
  });
  ...
  it('continues scanning all sources in dryRun mode after the first detection', async () => {
    ...
    expect(res.status).not.toHaveBeenCalled();
    expect(next).toHaveBeenCalledTimes(1);
    expect(req.fortifyjsDetections).toHaveLength(3);
    expect(req.fortifyjsDetections.map(event => event.path)).toEqual(expect.arrayContaining([
      'query.a',
      'body.comment',
      'cookies.next'
    ]));
  ...
  it('continues scanning request signals after a dryRun schema violation', async () => {
    ...
    expect(res.status).not.toHaveBeenCalled();
    expect(next).toHaveBeenCalledTimes(1);
    expect(req.fortifyjsDetections.map(event => event.label)).toEqual(expect.arrayContaining([
      'schema_violation',
      'xss'
    ]));
  ...
  it('blocks instead of throwing when request objects cannot be enumerated', async () => {
    ...
    expect(next).not.toHaveBeenCalled();
    expect(req.fortifyjs.reason).toBe('object_enumeration_failed');
  });
  ...
  it('blocks instead of throwing when request object properties throw on access', async () => {
    ...
    expect(next).not.toHaveBeenCalled();
    expect(req.fortifyjs.reason).toBe('object_property_access_failed');
  });
  ...
  it('reports schema violations instead of throwing when schema sources cannot be enumerated', async () => {
    ...
    expect(next).not.toHaveBeenCalled();
    expect(req.fortifyjs.reason).toBe('unreadable_object');
  });
  ...
  it('blocks instead of throwing when request source getters throw', async () => {
    ...
    expect(next).not.toHaveBeenCalled();
    expect(req.fortifyjs.reason).toBe('source_read_failed');
  });
  ...
  it('blocks instead of throwing when schema source getters throw', async () => {
    ...
    expect(next).not.toHaveBeenCalled();
    expect(req.fortifyjs.reason).toBe('schema_source_read_failed');
  });
  ...
  it.each([
    ...
    expect(res.status).toHaveBeenCalledWith(403);
    expect(next).not.toHaveBeenCalled();
    expect(req.fortifyjs.path).toBe(expectedPath);
  });
  ...
  it.each([
    ...
    expect(next).not.toHaveBeenCalled();
    expect(req.fortifyjs.reason).toBe('unexpected_field');
  });
  ...
  it('escapes newlines in text logs and request IDs', async () => {
    ...
    const req = {
      headers: {
        'x-request-id': 'req-123\n[fortifyjs] forged request'
      },
      query: {
        q: "1' OR '1'='1\n[fortifyjs] Attack Blocked: forged_entry from IP: 127.0.0.1"
      }
    };
    const res = createMockResponse();
    const next = jest.fn();
    ...
    expect(logs[0]).toContain("'1'='1\\n[fortifyjs] Attack Blocked: forged_entry");
  });
  ...
  it('falls back safely when request ID or rate-limit key callbacks throw', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(403);
    expect(next).not.toHaveBeenCalled();
    expect(req.fortifyjs.requestId).toBeNull();
    expect(req.fortifyjs.ip).toBe('203.0.113.10');
