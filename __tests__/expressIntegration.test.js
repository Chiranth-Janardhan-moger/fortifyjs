...
const { createNestMiddleware, expressMiddleware, nestjsMiddleware, secureRouter, fortifyjs } = require('../src/index');
...
describe('Express integration', () => {
  ...
  it('guard.route skips request sources already scanned by guard.global', async () => {
    ...
    const guard = fortifyjs({ dryRun: true, onThreat });
  ...
  it('can inspect raw multipart form-data when the app captures rawBody before custom parsing', async () => {
    const app = express();
    const boundary = 'fortifyjsBoundary';
    const multipartBody = [
      `--${boundary}`,
  ...
  it('records suspicious allowed payloads in safe learning mode', async () => {
    ...
    expect(learningEvents[0].clusterKey).toMatch(/^anomaly:/);
  });
