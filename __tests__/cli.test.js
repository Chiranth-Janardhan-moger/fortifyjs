...
const cliPath = path.join(__dirname, '..', 'bin', 'fortifyjs.js');
...
describe('CLI scanner', () => {
  ...
  it('scans payload files and emits CSV', () => {
    const filePath = path.join(os.tmpdir(), `fortifyjs-cli-${process.pid}.txt`);
  ...
  it('sanitizes formula-leading payloads in CSV output', () => {
    ...
    expect(output).toContain('payload,label,confidence');
    expect(output).toContain('"\'=HYPERLINK(""http://evil.test"",""click"")","anomaly",0.4');
  });
